...
describe("AI agent tool layer", () => {
  ...
  it("create_diagram labels the edges that ask for it, and leaves the rest bare", () => {
    // The prompt routes anything diagram-shaped here, so a label supported only
    // on bind_arrow is one the model can hardly ever reach. Worse, it does not
    // fail loudly: the agent reported back that it had labelled an arrow it had
    // not labelled.
    create_diagram(api, {
      nodes: [
        { key: "api", label: "API" },
    ...
    const labelled = arrows.filter((a) => a.label);
    expect(labelled).toHaveLength(1);
    expect(labelled[0].label).toBe("reads from");

    // No orphaned text: every label belongs to its arrow.
    expect(scene.filter((el) => el.type === "text")).toHaveLength(0);
  });

  it("bind_arrow labels the arrow, and the label really lands in the scene", () => {
    const a = add_shape(api, {
      x: 0,
      y: 0,
      width: 180,
      height: 80,
    ...
    expect(arrow.bound).toBe(true);
    expect(arrow.label).toBe("reads from");

    // The label is a separate text element bound back by containerId, not a
    // field on the arrow. Committing only the arrow would leave it claiming a
    // boundElements entry for a text element that is not in the scene.
    const elements = api.getSceneElements();
    const text: any = elements.find(
      (el: any) => el.type === "text" && el.containerId === arrow.id,
    );
    expect(text).toBeDefined();
    expect(text.text).toBe("reads from");

    const arrowEl: any = elements.find((el) => el.id === arrow.id);
    expect(arrowEl.boundElements.map((x: any) => x.id)).toContain(text.id);
  });

  it("an arrow label takes the arrow's colour and reads smaller than a box label", () => {
    const a = add_shape(api, { x: 0, y: 0, width: 180, height: 80, label: "A" });
    const b = add_shape(api, {
      x: 400,
      y: 0,
      width: 180,
    ...
    const text: any = api
      .getSceneElements()
      .find((el: any) => el.type === "text" && el.containerId === arrow.id);

    // Otherwise a coloured flow gains a near-black word in the middle of it.
    expect(text.strokeColor).toBe("#1971c2");
    // Subordinate to the boxes it runs between (which use Excalidraw's 20).
    expect(text.fontSize).toBeLessThan(20);
  });

  it("an unlabelled arrow gains no text element", () => {
    const a = add_shape(api, { x: 0, y: 0, width: 180, height: 80, label: "A" });
    const b = add_shape(api, {
      x: 400,
      y: 0,
    ...
    expect(
      api
        .getSceneElements()
        .filter((el: any) => el.type === "text" && el.containerId === arrow.id),
    ).toHaveLength(0);
  });

  it("get_scene reports an arrow's label, so the model can read back what it wrote", () => {
    const a = add_shape(api, { x: 0, y: 0, width: 180, height: 80, label: "A" });
    const b = add_shape(api, {
      x: 400,
      y: 0,
    ...
    const scene = get_scene(api);
    const seen = scene.find((el) => el.id === arrow.id);

    // The label must not also appear as a loose text element of its own, or the
    // model sees a floating word it might try to move or delete.
    expect(seen?.label).toBe("publishes to");
    expect(scene.filter((el) => el.type === "text")).toHaveLength(0);
  });
